(function (ext) {
    ...
    ext.Adjust_stream_volume = function (ip, p1, callback) {
        console.log("Adjust_stream_volume");
        console.log(ip);
        console.log(p1);
        $.ajax({
            url: 'http://' + ip + port + '/?name=Adjust_stream_volume' + '&p1=' + p1,
            dataType: 'jsonp',
            crossDomain: true,
            success: function (data) {
                console.log("success handler");
    ...
    var descriptor = {
        blocks: [
            ['r', '設定目標IP: %s', 'Setting_targetIP', "192.168.xx.xx"],
            ...
            ['', 'IP %s 調整說話音量 %m.volume_type', 'Adjust_stream_volume', "192.168.0.1", '大聲點'],
        ],
        menus: {
